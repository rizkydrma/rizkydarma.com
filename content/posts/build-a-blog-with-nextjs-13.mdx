...
banner: '/banner/u67wtugwwcxwl1orm9t2'
...
export default function Header() {
  return (
    <header>
      ...
      // If you want to add a Client Component here like a theme switcher, mark that // component with "use client" and leave
      the majority of the header as an RSC
    </header>
  );
}
```
...
<Note>
  If you think it's bad that we call `getPost` _and_ `getPosts()`, remember the we wrapped `getPosts` in `cache`.
  `getPost` just calls `getPosts`, so we're not making any unnecessary requests to the filesystem (or wherever you're
...
const main = () => {
  ...
  posts.forEach((post) => {
    ...
    feed.item({
      ...
      publishedAt: '2021-03-11'new Date(post?.date),
      author: 'Max Leiter',
      url,
      guid: url,
    });
